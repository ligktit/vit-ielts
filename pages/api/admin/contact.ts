...
export default async function handler(
  ...
  if (req.method === "GET") {
    try {
      const config = await readConfig<ContactPageConfig>(supabaseAdmin, sectionName);
  ...
  if (req.method === "POST") {
    try {
      const body = req.body as ContactPageConfig;
      await writeConfig<ContactPageConfig>(supabaseAdmin, sectionName, body);
